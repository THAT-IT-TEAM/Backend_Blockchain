...
export default function ExpensesPage() {
  ...
  const fetchExpensesAndSchema = async () => {
    setLoading(true);
    setError("");
    try {
      ...
      const schema = await api.getTableSchema("expenses"); // Fetch schema for 'expenses' table
      ...
      setError("Failed to load expenses or schema: " + e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddRecord = async (recordIndex: number) => {
    ...
    try {
      ...
      await api.createExpense(cleanedRecord); // Use createExpense method in api.ts
      fetchExpensesAndSchema(); // Refresh data
  ...
  const handleDeleteRecord = async (recordId: string) => {
    if (
      ...
      setError("");
      try {
        await api.deleteRecord("expenses", recordId); // Use 'expenses' as table name
        fetchExpensesAndSchema(); // Refresh data
      } catch (e: any) {
        ...
        <div className="mb-4 p-4 border rounded-lg bg-card shadow-sm overflow-x-auto">
          ...
          <table className="min-w-full divide-y divide-gray-200 border border-border">
            ...
            <tbody className="bg-background divide-y divide-border">
              {/* Existing Data Rows */}
              {expenses.map((row: any, rowIndex: number) => (
                <tr
                  key={rowIndex}
